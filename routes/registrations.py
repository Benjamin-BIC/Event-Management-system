...
# Users cannot register more than once for the same event
    ...
    if existing_registration:
        raise HTTPException(
            status_code=409, detail="User has already registered for this event")

    new_registration = registration_service.create_registration(registration)
    if new_registration is None:
        raise HTTPException(
            status_code=500, detail="Failed to create registration")
...
@router.get("/{registration_id}/attended", response_model=Registration)
